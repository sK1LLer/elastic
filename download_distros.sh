...
func_download(){
mkdir -p "$working_dir"/distribs/"${distro_array[$i]}" && mkdir -p $working_dir/webdav/"$latest_version"/"${distro_array[$i]}"
...
if [[ ${distro_array[$i]} == winlogbeat ]]; then
  ...
  mv $working_dir/distribs/"${distro_array[$i]}"/"${distro_array[$i]}"-$latest_version-windows-x86_64.zip $working_dir/webdav/"$latest_version"/"${distro_array[$i]}"/
  ...
  mv $working_dir/distribs/"${distro_array[$i]}"/"${distro_array[$i]}"-$latest_version-amd64.deb $working_dir/webdav/"$latest_version"/"${distro_array[$i]}"/
else
